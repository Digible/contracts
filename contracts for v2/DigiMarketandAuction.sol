...
contract DigiMarketAndAuction is Ownable, ReentrancyGuard {
    ...
    *******************/
    ...
    mapping(address => mapping(uint256 => uint256))
        public lastAuctionByTokenByContract;
    mapping(address => mapping(uint256 => Royalty))
        public royaltiesByTokenByContract;
    ...
    constructor(address _digi, address _digiTrackAddress) public payable {
        require(
            address(_digi) != address(0) && _digiTrackAddress != address(0)
        );
        digiERC20 = _digi;
        ...
        returns (uint256)
    {
        ...
        if (
            royaltiesByTokenByContract[_nftContractAddress][_tokenId].wallet ==
            address(0)
        ) {
        ...
        inProgress(_auctionId)
    {
        require(marketLive, "Market closed");
        require(
            IERC20(sales[_auctionId].paymentCurrency).balanceOf(msg.sender) >
                sales[_auctionId].fixedPrice,
            "Not enough balance"
        );
        require(sales[_auctionId].fixedPrice > 0, "Direct buy unavailable");
        uint256 feeAmount = sales[_auctionId].fixedPrice.mul(purchaseFee).div(
            10000
        );
    ...
    function processPayment(
        ...
        if (sales[saleId].royalty) {
            royaltyFeeAmount = grossAmount
                .mul(
                    royaltiesByTokenByContract[
                        sales[saleId].nftContractAddress
                    ][sales[saleId].tokenId].fee
                )
                .div(10000);
        }
        ...
        string memory digiSafeStatus = DigiTrack(digiTrackAddress).getStatus(
            nftContractAddress,
            tokenId
        );
        ...
        if (
            ...
            keccak256(abi.encodePacked((digiSafeStatus))) ==
            keccak256(abi.encodePacked(("Digisafe - Redemption Ready"))) ||
            keccak256(abi.encodePacked((digiSafeStatus))) ==
            keccak256(abi.encodePacked(("Digisafe - Pending Grade")))
        ) {
            _receiverOfPayment = seller;
            _receiverOfRoyalty = royaltiesByTokenByContract[nftContractAddress][
                tokenId
            ].wallet;
            sales[saleId].paymentClaimed = true;
            sales[saleId].royaltyClaimed = true;
            accumulatedFeesByCurrency[
                paymentCurrency
            ] = accumulatedFeesByCurrency[paymentCurrency].add(feeAmount);
        }

        // 1. Send Payment to Seller or Escrow if Phygital and not in DigiSafe

        if (senderOfPayment != _receiverOfPayment) {
            if (sales[saleId].isAuction) {
                IERC20(paymentCurrency).transfer(_receiverOfPayment, netAmount);
        ...
        if (senderOfPayment != _receiverOfRoyalty) {
            if (royaltyFeeAmount > 0) {

                if (sales[saleId].isAuction) {
                 IERC20(paymentCurrency).transfer(_receiverOfRoyalty, royaltyFeeAmount);
        ...
        returns (uint256)
    {
        ...
        sales[newSaleId] = Sale({
            ...
            royalty: royaltiesByTokenByContract[_tokenAddress][_tokenId]
                .wallet != address(0),
            endDate: timeNow + _duration,
            paymentClaimed: false,
            royaltyClaimed: false,
        ...
        if (
            royaltiesByTokenByContract[_tokenAddress][_tokenId].wallet ==
            address(0)
        ) {
            royaltiesByTokenByContract[_tokenAddress][_tokenId].wallet = msg
                .sender;
        }

        emit NewSaleIdCreated(
            newSaleId,
            msg.sender,
            _tokenAddress,
            _tokenId,
    ...
    function claimPaymentAmount(uint256 saleId) external nonReentrant {
        ...
        uint256 feeAmount = sales[saleId].finalPrice.mul(purchaseFee).div(
            10000
        );
        uint256 netAmount = sales[saleId].finalPrice.sub(feeAmount).sub(
            royaltyFeeAmount
        );

        IERC20(sales[saleId].paymentCurrency).transfer(
            sales[saleId].owner,
            netAmount
        );
        accumulatedFeesByCurrency[
            sales[saleId].paymentCurrency
        ] = accumulatedFeesByCurrency[sales[saleId].paymentCurrency].add(
            feeAmount
        );
    }
    ...
    function claimRoyalty(uint256 saleId) external nonReentrant {
        ...
        uint256 royaltyFeeAmount = sales[saleId]
            .finalPrice
            .mul(
                royaltiesByTokenByContract[sales[saleId].nftContractAddress][
                    sales[saleId].tokenId
                ].fee
            )
            .div(10000);
     ...
     */
    function withdrawAcumulatedFees(address _currency)
        external
        nonReentrant
        onlyOwner
    {
